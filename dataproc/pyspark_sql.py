...
print("input:", input, "bq_output:", bq_output, "temp_bucket:", temp_bucket)
...
class DataTransformationManager:
    ...
    def data_frame_transformations(self):
        df_transformed = self.data_frame \
            ...
            .withColumn("skyc1", 
                        when(
                            self.data_frame["skyc1"].isin("FEW", "OVC", "SCT", "BKN", "NSC", "VV"), 
                            self.data_frame["skyc1"])
                            .otherwise(None)
            )
...
def main():
    ...
    for key, values in bq_tables_to_create.items():
        ...
        print("temp_table_name:", temp_table_name, "query:", query)
        ...
        print("query output head:", bq_table_create.pyspark_query.head())
